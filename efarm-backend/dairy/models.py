...
class Cow(models.Model):
    ...
    current_pregnancy_status = models.CharField(choices=CowPregnancyChoices.choices, default=CowPregnancyChoices.UNAVAILABLE, max_length=12)
    category = models.CharField(choices=CowCategoryChoices.choices, default=CowCategoryChoices.CALF, max_length=11,)
    current_production_status = models.CharField(choices=CowProductionStatusChoices.choices, max_length=22, default=CowProductionStatusChoices.CALF)
...
class Heat(models.Model):
    ...
    def clean(self):
        ...
        if self.cow.pregnancy_status == 'Calved' and self.observation_time - self.cow.pregnancy.calving_date < timedelta(
                days=60):
            raise ValidationError('Cow cannot be in heat within 60 days after calving.')

        if self.cow.heat_set.filter(
                observation_time__range=(timezone.now() - timedelta(days=21), timezone.now())).exists():
...
class Semen(models.Model):
    ...
    producer = models.CharField(max_length=64,
                                choices=(('KALRO', 'Kenya Agricultural and Livestock Research Organization'),
...
class Insemination(models.Model):
    ...
    date_of_insemination = models.DateField(validators=[MaxValueValidator(date.today())],
                                            error_messages={
                                                "max_value": "Invalid date entry, Dates must not be in future"})
...
class Culling(models.Model):
    ...
    MEDICAL_REASONS = [('disease', 'Disease'), ('illness', 'Illness'), ('injuries', 'Injuries'),
                       ('chronic_health', 'Chronic Health Issues'), ]
    FINANCIAL_REASONS = [('cost_of_care', 'Cost of Care'), ('unprofitable', 'Unprofitable'),
                         ('low_market_demand', 'Low Market Demand'), ]
    PRODUCTION_REASONS = [('low_production', 'Low Production'), ('poor_quality', 'Poor Quality'),
                          ('inefficient_feed', 'Inefficient Feed Conversion'), ]
    GENETIC_REASONS = [('inherited_diseases', 'Inherited Diseases'), ('inbreeding', 'Inbreeding'),
                       ('unwanted_traits', 'Unwanted Traits'), ]
    ENVIRONMENTAL_REASONS = [('climate_change', 'Climate Change'), ('natural_disasters', 'Natural Disasters'),
                             ('overpopulation', 'Overpopulation'), ]
    LEGAL_REASONS = [('government_regulations', 'Government Regulations'),
